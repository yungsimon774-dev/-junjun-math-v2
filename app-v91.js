...
function extractArithmetic(q){
 const text=(q.explain||"")+" "+(q.prompt||"");
 let m=text.match(/(\d{1,4})\s*([+\-×÷])\s*(\d{1,4})\s*=\s*(\d{1,5})/);
 if(m)return {a:+m[1],op:m[2],b:+m[3],ans:+m[4]};
 m=text.match(/(\d{1,4})\s*([+\-×÷])\s*(\d{1,4})/);
 if(m)return {a:+m[1],op:m[2],b:+m[3],ans:null};
 return null;
}
...
function buildArithmeticLesson(x,q){
 const {a,b,op}=x, answer=x.ans!=null?x.ans:q.options[q.answer];
 let steps=[],visual="";
 if(op==="+"){
   const ao=a%10,bo=b%10,at=Math.floor(a/10),bt=Math.floor(b/10);
   steps.push(`先看题目：${a} 加 ${b}。`);
   if(a<21&&b<21){steps.push(`从 ${a} 开始，向右数 ${b} 步。`);steps.push(`最后走到 ${answer}。`);visual=numberLineHTML(a,b,"+")}
   else if(ao+bo>=10){
     steps.push(`先算个位：${ao}+${bo}=${ao+bo}。`);
     steps.push(`${ao+bo}里面有1个十和${(ao+bo)%10}个一，所以个位写${(ao+bo)%10}，向十位进1。`);
     steps.push(`再算十位：${at}+${bt}+1=${Math.floor(Number(answer)/10)}。`);
     steps.push(`所以答案是 ${answer}。`);visual=placeValueHTML(answer)
   }else{
     steps.push(`先算个位：${ao}+${bo}=${ao+bo}。`);
     steps.push(`再算十位：${at}+${bt}=${at+bt}。`);
     steps.push(`合起来是 ${answer}。`);visual=placeValueHTML(answer)
   }
 }else if(op==="-"){
   const ao=a%10,bo=b%10,at=Math.floor(a/10),bt=Math.floor(b/10);
   steps.push(`先看题目：${a} 减 ${b}。`);
   if(a<21&&b<21){steps.push(`从 ${a} 开始，向左退 ${b} 步。`);steps.push(`最后走到 ${answer}。`);visual=numberLineHTML(a,b,"-")}
   else if(ao<bo){
     steps.push(`个位上 ${ao} 不够减 ${bo}。`);
     steps.push(`从十位借1个十，${ao}变成${ao+10}。`);
     steps.push(`个位：${ao+10}-${bo}=${ao+10-bo}。`);
     steps.push(`十位借走1后再相减，最后得到 ${answer}。`);visual=placeValueHTML(answer)
   }else{
     steps.push(`先算个位：${ao}-${bo}=${ao-bo}。`);
     steps.push(`再算十位：${at}-${bt}=${at-bt}。`);
     steps.push(`所以答案是 ${answer}。`);visual=placeValueHTML(answer)
   ...
   steps=[`乘法表示几个相同的数相加。`,`把 ${b} 看成一组，共有 ${a} 组。`,`也就是 ${b} 连加 ${a} 次，结果是 ${answer}。`];
 }else{
   steps=[`除法表示平均分。`,`把 ${a} 平均分成 ${b} 份。`,`每份是 ${answer}。`];
 }
 return {steps,visual};
}
