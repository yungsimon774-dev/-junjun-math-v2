...
let answerMode=localStorage.getItem("junjun_answer_mode")||"mixed", currentQuestionMode="choice", typedAnswer="";
...
function numericCorrectAnswer(q){const raw=q&&q.options&&q.options[q.answer];if(raw==null)return null;const t=String(raw).trim().replace(/，/g,",");if(!/^-?\d+(?:\.\d+)?$/.test(t))return null;return Number(t)}
function chooseQuestionMode(q){if(answerMode==="choice")return "choice";if(answerMode==="input")return numericCorrectAnswer(q)!==null?"input":"choice";return numericCorrectAnswer(q)!==null&&((idx%2===0)||Math.random()<.5)?"input":"choice"}
function renderModeControl(){const names={mixed:"自动混合",input:"只做填空",choice:"只做选择"};const el=$("#answerModeText");if(el)el.textContent=names[answerMode]||names.mixed;$$('[data-answer-mode]').forEach(b=>b.classList.toggle('active',b.dataset.answerMode===answerMode))}
function setAnswerMode(mode){answerMode=["mixed","input","choice"].includes(mode)?mode:"mixed";localStorage.setItem("junjun_answer_mode",answerMode);renderModeControl();if(course&&qs.length)show()}
function inputHTML(){return `<div class="fill-answer-wrap"><div class="fill-label">请自己输入答案，不能靠猜</div><div id="typedDisplay" class="typed-display">＿</div><div class="number-pad">${[1,2,3,4,5,6,7,8,9].map(n=>`<button class="num-key" data-num="${n}">${n}</button>`).join("")}<button class="num-key action-key" data-action="clear">清除</button><button class="num-key" data-num="0">0</button><button class="num-key action-key submit-key" data-action="submit">确定</button></div></div>`}
function updateTypedDisplay(){const el=$("#typedDisplay");if(el)el.textContent=typedAnswer||"＿"}
function submitTyped(){if(locked||typedAnswer==="")return;const q=qs[idx],correct=numericCorrectAnswer(q);gradeAnswer(Number(typedAnswer)===correct,null,typedAnswer)}
function show(){locked=false;typedAnswer="";closeTeacher();const q=qs[idx];currentQuestionMode=chooseQuestionMode(q);$("#gameTitle").textContent=course.icon+" "+course.title;$("#count").textContent=(idx+1)+"/"+qs.length;$("#fill").style.width=(idx/qs.length*100)+"%";$("#question").textContent=q.prompt;$("#options").innerHTML=currentQuestionMode==="input"?inputHTML():q.options.map((x,i)=>`<button class="opt" data-i="${i}">${x}</button>`).join("");$("#feedback").textContent="题型："+(q.kind||"同步训练")+"｜难度："+(q.difficulty||"提高")+"。"+(currentQuestionMode==="input"?"请自己输入数字答案。":"请选择答案。");$("#next").classList.add("hidden");$$('.opt').forEach(b=>b.onclick=()=>answer(+b.dataset.i));$$('.num-key').forEach(b=>b.onclick=()=>{if(locked)return;if(b.dataset.num!=null){if(typedAnswer.length<7)typedAnswer+=b.dataset.num;updateTypedDisplay()}else if(b.dataset.action==="clear"){typedAnswer="";updateTypedDisplay()}else if(b.dataset.action==="submit")submitTyped()});renderModeControl()}
function answer(i){if(locked)return;const q=qs[idx];gradeAnswer(i===q.answer,i,String(q.options[i]??""))}
function gradeAnswer(ok,selectedIndex,entered){if(locked)return;locked=true;const q=qs[idx];ensureDaily();st.attempts++;st.daily.attempts++;sessionAttempts++;const cs=st.courseStats[course.id]||(st.courseStats[course.id]={attempts:0,correct:0,rounds:0,best:0});cs.attempts++;$$('.opt').forEach((b,n)=>{b.disabled=true;if(n===q.answer)b.classList.add("good");if(n===selectedIndex&&!ok)b.classList.add("bad")});$$('.num-key').forEach(b=>b.disabled=true);if(ok){st.correct++;st.daily.correct++;st.stars+=2;session++;cs.correct++;if(course.id==="mistake_retry"&&q.mistakeKey)st.mistakes=st.mistakes.filter(m=>m.key!==q.mistakeKey);$("#feedback").innerHTML="✅ 答对了！"+esc(q.explain||"");if(currentQuestionMode==="input")$("#typedDisplay")?.classList.add("good-input");play("correct");celebrate(12);say("答对了，真厉害")}else{const key=[course.id,q.prompt,(q.options||[]).join("|")].join("::");const item={key,courseId:course.id,course:course.title,q:q.prompt,options:[...(q.options||[])],answer:q.answer,a:q.options[q.answer],explain:q.explain||"",kind:q.kind||"综合题",difficulty:q.difficulty||"巩固",wrongAt:Date.now()};st.mistakes=[item,...st.mistakes.filter(m=>(m.key||[m.courseId,m.q].join("::"))!==key)].slice(0,50);if(currentQuestionMode==="input")$("#typedDisplay")?.classList.add("bad-input");$("#feedback").innerHTML="💡 你填的是：<b>"+esc(entered||"未填写")+"</b><br>正确答案：<b>"+esc(q.options[q.answer])+"</b><br>点“教我”看一步一步讲解。<br>"+esc(q.explain||"");play("wrong");say("差一点，点教我，我们一步一步来")}save();$("#next").classList.remove("hidden")}
...
document.addEventListener("DOMContentLoaded",()=>{
 ...
 $$(`[data-answer-mode]`).forEach(b=>b.onclick=()=>setAnswerMode(b.dataset.answerMode));renderModeControl();
