...
let voiceGender=voiceProfile.split("-").at(-1)==="male"?"male":"female";
...
function voicesFor(lang){
 ...
 return voices.filter(v=>{
   ...
   if(target==="zh-cn") return l==="zh-cn"||l.startsWith("zh-cn-")||l==="cmn-cn"||l==="zh-hans-cn";
   if(target==="zh-hk") return l==="zh-hk"||l.startsWith("zh-hk-")||l==="yue-hk"||l==="zh-hant-hk";
   return l===target;
 });
}
function pickVoice(lang,gender){
 ...
 const names=gender==="male"
   ? ["yunxi","yunyang","yunze","yun jian","kangkang","daniel","aaron","li-mu","li mu"]
   : ["tingting","ting-ting","xiaoxiao","huihui","yaoyao","meijia","mei-jia","sinji","sin-ji","sandy","lina","li na"];
 for(const key of names){
   const hit=list.find(v=>(v.name+" "+v.voiceURI).toLowerCase().includes(key));
   if(hit)return hit;
 }
 // iOS/Safari很多设备每种中文只提供一个系统声音；此时保证语言正确，不跨语言回退。
 return list[0];
}
...
function updateVoiceStatus(){
 ...
 el.textContent=`选择：${voiceLang==="zh-HK"?"广东话":"普通话"}·${voiceGender==="male"?"男声":"女声"}｜系统实际：${voiceLabel(v)}${v&&voiceGender==="male"&&!/male|yunxi|yunyang|yunze|kangkang|daniel|aaron/i.test(v.name)?"（本机没有对应男声，使用同语言可用声音）":""}`;
}
function applyVoiceProfile(profile){
 voiceProfile=profile;
 voiceLang=profile.startsWith("zh-HK-")?"zh-HK":"zh-CN";
 voiceGender=profile.split("-").at(-1)==="male"?"male":"female";
...
function say(t,onDone){
 ...
 const v=pickVoice(voiceLang,voiceGender);
 if(v){u.voice=v;u.lang=v.lang||voiceLang;} // 明确绑定同语言系统声音
